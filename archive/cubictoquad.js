"use strict";
// https://github.com/fontello/cubic2quad/tree/master
...
function processSegment(a, b, c, d, t1, t2) {
  ...
  var cx =
    (f1_.x * (f2.y * f2_.x - f2.x * f2_.y) +
      f2_.x * (f1.x * f1_.y - f1.y * f1_.x)) /
    D;
  var cy =
    (f1_.y * (f2.y * f2_.x - f2.x * f2_.y) +
      f2_.y * (f1.x * f1_.y - f1.y * f1_.x)) /
    D;
 ...
 *  - errorBound is maximum allowed distance
 */
function isSegmentApproximationClose(
  a,
  b,
  c,
  d,
...
function cubicToQuad(p1x, p1y, c1x, c1y, c2x, c2y, p2x, p2y, errorBound) {
  ...
  for (
    var inflectionIdx = 0;
    inflectionIdx < inflections.length;
    inflectionIdx++
  ) {
